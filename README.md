...
L'indice de confiance correspond au nombre minimal de point observé par région et par an pour réaliser l'estimation de la dynamique des fréquences. Lorsque cette moyenne est strictement en dessous de l'indice de confiance souhaité dans une région donnée, alors tous les points observés dans cette région sont retirés de l'analyse. En effet, une trop faible répétition de l'échantillonage dans le temps enentrainerait une estimation peu robuste de la dynamique.
L'ajout et le retrait des données de certaines régions peuvent jouer sur l'estimation des taux de croissance nationaux et régionnaux. C'es à l'utilisateur de faire son choix et de trouver la cohérence dans les estimations en essayant plusieurs seuils de confiance.
...
Exemple : Si on a +0.86 (\*\*\*) en FRANCE, -0.5 () dans REG1 et +0.32 (\*) dans REG2, cela signifie que la fréquence augmente de manière significative en FRANCE (**+0.86**), que la dynamique de la fréquence dans REG1 ne diffère pas significativement de cette tendance nationale (+0.68-0.5=**+0.36**), que la dynamique de la fréquénce dans la REG2 diffère significativement de cette tendance national (+0.86+0.32=**+1.18**). 
